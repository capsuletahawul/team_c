/**
 * Contact.jsx — Single Unified Component
 * Optimized to match StudentProfile architectural pattern.
 * Reads seamlessly from LanguageContext and utilizes centralized copy.js.
 * Line count under 200 lines.
 */

import React, { useState, useEffect } from 'react';
import { EnvelopeIcon, MapPinIcon, ClockIcon, PaperAirplaneIcon, ExclamationCircleIcon, CheckCircleIcon, XMarkIcon } from '@heroicons/react/24/outline';
...
export default function Contact() {
  ...
  const c = t.contact; // الوصول المباشر لترجمات صفحة الاتصال من الملف العام
  const isRTL = lang === 'ar';

  // Form & UI States
  const [formData, setFormData] = useState({ fullName: '', email: '',  phone: '', subject: '', category: '', message: '' });
  ...
  tempErrors.fullName = c.form.errRequired;
} else if (formData.fullName.trim().length < 3) {
  tempErrors.fullName = c.form.errMinName;
}
...
if (!formData.phone.trim()) {
  tempErrors.phone = c.form.errRequired;
} else if (!phoneRegex.test(formData.phone)) {
  tempErrors.phone = c.form.errPhone;
}
    ...
    <div className="min-h-screen bg-gray-50 font-sans flex flex-col" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <section className="relative w-full overflow-hidden bg-gradient-to-br from-[#164961] via-[#1a5570] to-[#387B84] py-16 text-center text-white">
        ...
        <div className="absolute bottom-0 left-0 right-0">
          <svg viewBox="0 0 1440 60" fill="none" className="w-full h-auto" preserveAspectRatio="none"><path d="M0 60h1440V20c-360 40-1080-40-1440 0v40z" fill="#F9FAFB"/></svg>
        </div>
      </section>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-xl shadow-slate-200/40 p-6 sm:p-8 space-y-6">
          ...
          {showAlert && (
            <div className="w-full bg-green-50 border border-green-500 rounded-xl p-4 shadow-sm flex items-start gap-3 transition-all duration-300" role="alert">
              ...
              <div className="flex-1"><h3 className="text-base font-bold text-green-800 mb-0.5">{c.form.alertTitle}</h3><p className="text-xs text-green-700">{c.form.alertDesc}</p></div>
              <button onClick={() => setShowAlert(false)} className="p-1 rounded-lg text-green-700 hover:bg-green-100"><XMarkIcon className="w-5 h-5" /></button>
            </div>
          )}

{submitError && (
  <div className="w-full bg-red-50 border border-red-400 text-red-700 rounded-xl p-4 mb-4">
    {submitError}
  </div>
)}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input type="text" value={formData.fullName} onChange={(e) => setFormData({...formData, fullName: e.target.value})} placeholder={c.form.fullNamePlaceholder} className={`w-full px-4 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-capsule-teal transition-all ${errors.fullName ? 'border-red-500 bg-red-50/30' : 'border-slate-200 bg-white'}`} />
            ...
            <div>
              ...
              <input type="text" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} placeholder={c.form.emailPlaceholder} className={`w-full px-4 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-capsule-teal transition-all ${errors.email ? 'border-red-500 bg-red-50/30' : 'border-slate-200 bg-white'}`} />
  ...
  <input
    ...
    className={`w-full px-4 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-capsule-teal transition-all ${
      errors.phone
      ...
      {errors.phone}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={formData.subject} onChange={(e) => setFormData({...formData, subject: e.target.value})} placeholder={c.form.subjectPlaceholder} className={`w-full px-4 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-capsule-teal transition-all ${errors.subject ? 'border-red-500 bg-red-50/30' : 'border-slate-200 bg-white'}`} />
              ...
              <div>
                ...
                <select value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})} className={`w-full px-4 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-capsule-teal transition-all ${errors.category ? 'border-red-500 bg-red-50/30' : 'border-slate-200 bg-white'}`}>
            ...
            <div>
              ...
              <textarea rows="4" value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} placeholder={c.form.messagePlaceholder} className={`w-full px-4 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-capsule-teal transition-all ${errors.message ? 'border-red-500 bg-red-50/30' : 'border-slate-200 bg-white'}`}></textarea>
